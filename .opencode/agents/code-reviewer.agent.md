...
### Angular/RxJS Patterns (HIGH)

When reviewing Angular code, check:
- **RxJS Memory Leaks** — Missing `takeUntil`, `DestroyRef`, or `async` pipe subscriptions that aren't cleaned up.
- **Change Detection** — Not using `ChangeDetectionStrategy.OnPush` where appropriate, causing unnecessary re-renders.
- **Direct DOM Manipulation** — Using `document.getElementById` instead of `Renderer2` or `@ViewChild`.
- **Heavy computations in templates** — Calling component methods directly in the HTML template (e.g., `*ngIf="isUserValid()"`) instead of using Signals or Pipes.
- **Missing Error Handling in Observables** — HTTP calls or streams without `catchError`.
- **Nested Subscriptions** — Subscribing inside another subscribe instead of using higher-order mapping operators (`switchMap`, `mergeMap`).

```typescript
// BAD: Memory leak, missing unsubscription
ngOnInit() {
  this.userService.getUsers().subscribe(users => this.users = users);
}

// GOOD: Async pipe handling subscription automatically
