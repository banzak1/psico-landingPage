...
- **Branch `main`**: Apenas para código estável e pronto para produção.
- **Branch `dev`**: Para o desenvolvimento de novas implementações e refatorações. Todo o foco de novos épicos, correções e testes maiores deve acontecer nesta branch antes de ir para a `main`.
...
## Base de Conhecimento (Obsidian Local)
- A pasta `psico/` atua como o *vault* (Obsidian) local do projeto. Ela deve ser a principal fonte de verdade para o contexto.
- **Épicos e Planejamentos Ágeis:** Devem ser sempre documentados e mantidos atualizados na pasta `psico/epics/` (ex: `psico/epics/epic-01-dashboard-admin.md`).
- **Produção de Conteúdo e Tom de Voz:** Textos para redes sociais (LinkedIn, X, etc.) e artigos criados com as skills da pasta `.agents` (como a `content-engine`) devem ser salvos na pasta `psico/content/` para servirem de histórico e manterem a consistência do tom de voz em conteúdos futuros. Sempre consulte esses arquivos antigos para mimetizar o tom.
